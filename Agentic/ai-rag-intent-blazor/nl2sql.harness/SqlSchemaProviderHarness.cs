...
public sealed class SqlSchemaProviderHarness
{
    ...
    public async Task ReverseEngineerSchemaAsync()
    {
        ...
        {
            var desc = Harness.Configuration.GetSection("DatabaseDescriptions")[dbKey.Key];
            var conn = dbKey.Value;
            await this.CaptureSchemaAsync(dbKey.Key, conn, desc).ConfigureAwait(false);
        }
    ...
    private async Task CaptureSchemaAsync(string databaseKey, string? connectionString, string? description, params string[] tableNames)
    {
